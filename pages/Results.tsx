...
import { Loader2, Info, Moon, Sun, X, CheckCircle2, Share2, Copy } from 'lucide-react';
...
`.trim();
  ...
  const handleShare = async () => {
    ...
    const shareText = `✨ Мой персональный разбор кожи от SkinGlow AI ✨\n\n` +
      `Тип кожи: ${box.data.skinType}\n` +
      `Стратегия: ${box.recommendation.strategy}\n\n` +
      `🧴 Мой бокс:\n` +
      box.products.map(p => `• ${p.name} (${p.brand})`).join('\n') +
      `\n\nИтого: ${box.totalPrice} ₽`;

    if (navigator.share) {
      try {
        await navigator.share({
          title: 'SkinGlow AI Результат',
          text: shareText,
        });
      } catch (err) {
    ...
    <div className="max-w-5xl mx-auto px-4 py-12">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-4">
        <div className="text-center md:text-left">
          <div className="inline-block bg-teal-100 text-teal-700 px-4 py-1 rounded-full text-sm font-semibold mb-2">
            Анализ завершен
          </div>
        ...
        <button 
          onClick={handleShare}
          className="flex items-center gap-2 px-5 py-2.5 bg-white border border-stone-200 rounded-full text-stone-600 hover:bg-stone-50 hover:text-teal-600 transition-colors shadow-sm font-medium"
        >
